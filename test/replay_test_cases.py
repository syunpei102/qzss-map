...
CASES.append(eew("EEW 取消", "茨城県沖", 36.3, 140.9, 3.9, [59], cancel=True))
...
CASES.append(tsunami("津波警報解除", 2, "警報解除", [310], ["千葉県九十九里・外房"], cancel_info=True))
...
CASES.append(jalert("Jアラート: 解除(先に出したミサイル発射と同じ種別/地域)", "Missile attack", "Extreme - Extraordinary threat to life or property", ["北海道", "青森県"], message_type="All Clear"))
...
# --- Lアラート 市区町村ケース(8件) ---
CASES.append(lalert_municipality("Lアラート(市区町村): 地すべり Extreme 札幌市中央区", "Landslide", "Extreme - Extraordinary threat to life or property", "札幌市中央区", 1101, "直ちに命を守るための最善の行動を。", "6 to 12 hours"))
CASES.append(lalert_municipality("Lアラート(市区町村): 大雨 Severe 那覇市", "Rainfall", "Severe - Significant threat to life or property", "那覇市", 47201, "低い土地からの避難を検討してください。", "1 to 6 hours"))
CASES.append(lalert_municipality("Lアラート(市区町村): 高潮 Extreme 名古屋市中区", "Coastal flooding", "Extreme - Extraordinary threat to life or property", "名古屋市中区", 23106, "沿岸部・河口付近は危険です。", "6 to 12 hours"))
CASES.append(lalert_municipality("Lアラート(市区町村): 大雪 Moderate 新潟市中央区", "Snowfall", "Moderate - Possible threat to life or property", "新潟市中央区", 15102, "不要不急の外出は控えてください。", "12 to 24 hours"))
CASES.append(lalert_municipality("Lアラート(市区町村): 暴風雪 Severe 函館市", "Snow storm / blizzard", "Severe - Significant threat to life or property", "函館市", 1202, "視界不良に警戒してください。", "6 to 12 hours"))
CASES.append(lalert_municipality("Lアラート(市区町村): ダム決壊 Extreme 福岡市博多区", "Dam failure or bursting of a dam", "Extreme - Extraordinary threat to life or property", "福岡市博多区", 40133, "直ちに命を守るための最善の行動を。", "1 to 6 hours"))
CASES.append(lalert_municipality("Lアラート(市区町村): 訓練 Unknown 仙台市青葉区", "Safety warning", "Unknown", "仙台市青葉区", 4101, "これは訓練です。", ""))
CASES.append(lalert_all_clear("Lアラート 解除: 地すべり(札幌市中央区)", "Landslide", area_code=1101))
...
CASES.append(weather("気象: 警報解除(東京地方)", [(130010, "東京地方")], ["大雨特別警報"], cancel=True))
...
CASES.append(other_category("洪水: 取消", 11, "洪水警報", "利根川の洪水警報を解除しました。", cancel=True))
...
def main():
    ...
    print(f"📦 テストケース数: {total} 件 / 間隔: {INTERVAL_SEC}秒")
    print(f"⏱️  想定所要時間: 約{int(total * INTERVAL_SEC / 60)}分")
    print("-" * 60)
