...
- `ADMIN_EMAIL` / `ADMIN_PASSWORD_HASH` / `SESSION_SECRET` を環境変数に設定する
  (未設定の間は`/device-admin`関連の機能が無効になるだけで、地図本体には影響しない)
...
`https://<デプロイ先>/device-admin` に、メールアドレス+パスワードでログインすると、

- 登録済みラズパイ(拠点)ごとの温度・稼働時間・ディスク空き・オンライン状態を確認できる
- 「再起動を予約」「更新確認を予約」で遠隔操作できる(ラズパイは外部からの着信を一切
  受け付けない設計のため、次回の定期報告時にラズパイ側から取りに来る形で反映される)
- 拠点ごとに「対象地域」(都道府県)を割り当てられる。割り当てると、その都道府県+
  周辺地方(例: 東京→関東)だけにその拠点の表示・送信が固定される

ログイン用の資格情報は環境変数 `ADMIN_EMAIL` / `ADMIN_PASSWORD_HASH` / `SESSION_SECRET`
で設定する(複数アカウントには対応していない、運用者本人のみの想定)。
`ADMIN_PASSWORD_HASH` は平文パスワードをそのまま環境変数に置かないためのハッシュ値で、
以下のように生成する。

```bash
cd map
node hash_admin_password.js 'あなたのパスワード'
# 出力された salt:hash 文字列を ADMIN_PASSWORD_HASH に設定する
```

拠点(ラズパイ)ごとの地域ロックを使う場合、その拠点専用の表示用URLとして
`https://<デプロイ先>/?device=<拠点ID>` を使う(`<拠点ID>` はラズパイの`qzss.env`の
`QZSS_DEVICE_ID`と一致させる)。`?device=`を付けない通常の公開URLは今まで通り全国対象・
絞り込みなしのまま。
...
- `/device-admin` はメールアドレス+パスワード(`ADMIN_EMAIL`/`ADMIN_PASSWORD_HASH`)で
  ログインし、HMAC署名付きCookie(`SESSION_SECRET`)でセッションを保持する。
  パスワードは平文で保存・比較しない。
- `INGEST_TOKEN` / `ADMIN_PASSWORD_HASH` / `SESSION_SECRET` はすべてCloud Runの環境変数
  としてのみ保持し、**このリポジトリは公開リポジトリなので、コード・スクリプト・
  ドキュメントのどこにも実際の値を書かないこと。** ローカルで試す場合も、シェルの
  環境変数として渡すか未追跡ファイル(`.gitignore`対象)に置くこと。
