...
最終結果は，地図側の自動テスト65件とPi側114件がすべて成功，`npm audit --omit=dev`は脆弱性0件である．ただし，修正はまだローカル作業ツリーだけにあり，コミット／push／デプロイは行っていない．実機固有の6項目も未確認なので，Issueはopenのまま維持した．
...
- コミット／push／PR／デプロイ：未実施．
- Issue：修正がリモートへ入っていないためopenを維持．CIと実機確認が完了したIssueからcloseする．
